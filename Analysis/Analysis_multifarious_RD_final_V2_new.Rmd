...
Dexiostoma <- dplyr::filter(nested_gams, species == "Dexiostoma")
...
unique_values %>% dplyr::filter(richness == 2) %>% drop_na() %>% 
...
mag_1<- read_csv("final_scenario_2_reanalysis.csv")
#View(mag_1)
...
max_row <- grouped_dd %>%
  dplyr::filter(mean_divergence == max(mean_divergence))


# Find the row with the minimum mean_divergence
min_row <- grouped_dd %>%
  dplyr::filter(mean_divergence == min(mean_divergence))
...
closest_to_mean_row <- grouped_dd %>%
  dplyr::filter(abs(mean_divergence - mean(mean_divergence)) == min(abs(mean_divergence - mean(mean_divergence))))
...
new_selection_mean<-dplyr::filter(sel_means,sel%in%selection_mean$sel)
...
get_RD_ours_alt <- function(species_names, nested_gams, E1_var, E2_var) {
  species_data <- nested_gams %>% dplyr::filter(species %in% species_names)
...
#write.csv(df_slopes,"df_slopes.csv")
...
new_selection_mean_2<-apply(new_selection_mean,1,function(r){
  slopes<-dplyr::filter(df_slopes,nutrients==r[8],
                 temperature==r[7],
...
get_deriv<-function(nutrient_level,sp){
  ...
  partial_df<-dplyr::filter(partial_df,species%in%sp)
...
selection_mean_single<-dplyr::filter(selection_mean,treatment=="Tmp_25_28_Nut0.01")
...
endpoints<-dplyr::filter(partial_df,temperature%in%t_range)
...
legends<-annotate_figure(legends,
                         top=text_grob(expression("Growth rate ["* day^{-1} *  "]"),size=15,vjust=1,hjust=1))


#run first surface plots without the guides on (don't rerun legends)

fig1A<-ggarrange(s_dexiostoma,s_loxocephalus,s_paramecium,s_spirostotum,s_colpidium,common.legend=F,nrow=2,ncol=3,align="hv")+
  theme(plot.margin = margin(0.1,0.1,0.1,1.5, "cm")) 



fig1A<-annotate_figure(fig1A,bottom=text_grob("Temperature [°C]",size=16),
                       left=text_grob("Nutrients [g/L]",size=16,vjust=2,rot=90))

ggsave("surfaces.pdf", plot = fig1A, width = 10, height = 6)
ggsave("surfaces_legend.pdf", plot = legends, width = 4, height = 3)
```
...
plot_regression_day <- ggplot(dd_fig, aes(x = day, y = log10(density), color = as.factor(nutrients))) +
  ...
  #geom_smooth(data = dd %>% dplyr::filter(day <= 6, species != "Spirostomum" & species != "Colpidium"&species != "Colpidium"), method = "lm", se = FALSE, aes(group = nutrients, color = factor(nutrients)), linewidth = 1.5) +  # LM line for days <= 5
  #geom_smooth(data = dd %>% dplyr::filter(species == "Spirostomum"), method = "lm", se = FALSE, aes(group = nutrients, color = factor(nutrients)), linewidth = 1.5) +  # LM line for Spirostomum across all days
...
dd_spiro<-dplyr::filter(dd,species=="Loxocephalus",day==1)
...
df_int <- gather(df_slopes%>%filter(nutrients=="0.35 g/L",temperature=="22-25 °C"), key = "source", value = "values", maxT_y, minT_y)
...
ggsave("mock_plot.pdf", plot = mock_plot, width = 10, height = 6)
