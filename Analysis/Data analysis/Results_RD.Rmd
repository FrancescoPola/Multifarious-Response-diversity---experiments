...
lm_async<-lm(data=async_aggr%>%filter(!is.na(synchrony_Gross),nutrients!="0.01 g/L"),log10(1+synchrony_Gross)~log10(abs_sum_slope))
...
sem_aggr <- async_aggr %>%
  ...
  mutate(
    log_abs_sum_slope = log10(sum_abs_slope),
...
interactions_df <- read_csv("interactions_df.csv")
...
M_int<-lm(data=int_aggr%>%dplyr::filter(mean_int_strength!=0),log10(stability)~log10(avg_w_sumslopes)+nutrients+temperature+richness+mean_interaction)
anova(M_int,lm_full)



plot_int_syn<-ggplot(data=int_aggr,aes(y=synchrony_Gross,x=mean_int_strength))+
  geom_point()+
  geom_smooth(method="lm")


M_int_syn<-lm(data=int_aggr%>%dplyr::filter(!is.na(synchrony_Gross)),synchrony_Gross~log10(abs_sum_slope)*richness+nutrients+temperature+mean_int_strength)


plot_richness_int<-ggplot(data=int_aggr,aes(x=mean_int_strength))+
  geom_histogram()+
  facet_wrap(~nutrients)

### sem

sem_aggr<-full_join(interactions_df,sem_aggr,join_by(sample_ID))
sem_aggr$mean_int_strength[is.na(sem_aggr$mean_int_strength)]=0
...
'
fit_int <- sem(model_int, estimator="MLM",meanstructure = TRUE,data = sem_aggr%>%dplyr::filter(!is.na(synchrony_Gross)))
