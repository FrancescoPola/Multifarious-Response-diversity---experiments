...
lm_full<-lm(data=async_aggr, log10(stability)~log10(abs_sum_slope)+richness+temperature+nutrients)
...
rel_imp <- calc.relimp(lm_full_div, type = "lmg", rela = TRUE)
...
model_asyn_balance<-lm(data=async_aggr%>%filter(!is.na(synchrony_Gross)),synchrony_Gross~log10(abs_sum_slope))
...
# Define the SEM model using lavaan syntax
 model2 <- '
  ...
  synchrony_Gross ~ log_abs_sum_slope

'

# Fit the model
fit <- sem(model2, data = sem_aggr%>%filter(nutrients!="0.01 g/L"))
fit <- sem(model, data = sem_aggr%>%filter(nutrients!="0.01 g/L"))
...
RD_int_plotRD_int_plot<-ggplot(data=async_aggr%>%filter(nutrients!="0.01 g/L"),aes(x=synchrony_Gross,y=log10(stability)))+
