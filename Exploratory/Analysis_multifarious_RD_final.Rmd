...
env_data_1$nutrients <- rep(c(0.01, 0.75, 1.25), each = total_days)
...
# Set temperature values based on the description
temperature_pattern <- c(rep(21, 3), rep(24, 3))
temperature_pattern <- rep(temperature_pattern, length.out = total_days)
...
# Set nutrients levels for each time point
env_data_2$nutrients <- rep(c(0.01, 0.75, 1.25), each = total_days)
...
# Set temperature values based on the description
temperature_pattern <- c(rep(24, 3), rep(27, 3))
temperature_pattern <- rep(temperature_pattern, length.out = total_days)
...
# Set nutrients levels for each time point
env_data_3$nutrients <- rep(c(0.01, 0.75, 2.5), each = total_days)
...
plot <- plot %>% layout(title=list(text=species_name,y=0.8,x=0.15),scene = list(camera = list(eye = list(x = 1.7, y = 1.7, z = 1.5)),
  ...
  yaxis = list(title =list(text="Temperature [°C]",font=list(size=13)), tickmode = "array", tickvals = seq(0,500,by=100), ticktext = seq(18,28,by=2),ticklabelposition="inside bottom",ticklen=0,showgrid=F,zeroline=F),
...
single_plot1<-single_plot+annotate("text",x = 2, y = 0.9, label = "temperature = 25-28 °C", size = 3, color = "black",hjust=0)+
...
fig2<-annotate_figure(fig2,
                      left=text_grob("nutrients [g/L]",size=11,rot=90,vjust=0.9),bottom=text_grob("temperature [°C]"))
...
fig3a<-annotate_figure(fig3a,bottom=text_grob("temperature [°C]",size=15),
...
fig3c<-ggarrange(dn_1_LS+ylab("directional derivative"),dn_1_CD+xlab("temperature [°C]"),dn_1_DS,ncol=3,nrow=1,align="h")
