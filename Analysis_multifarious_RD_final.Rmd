...
closest_to_mean_row <- grouped_dd %>%
  mutate(mean_value = mean(mean_divergence)) %>%
...
selection_final <- bind_rows(max_row, min_row, closest_to_mean_row)
...
#Quite a few communities have exactly the same divergence values

ggplot(selection_final,aes(x=richness,y=mean_divergence,label=composition))+
  geom_point()+
